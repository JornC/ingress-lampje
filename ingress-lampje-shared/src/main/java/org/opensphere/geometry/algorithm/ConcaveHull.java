...
public class ConcaveHull {
   ...
   * @param threshold
   */
  public ConcaveHull(final GeometryCollection geometries, final double threshold) {
   ...
   */
  private static GeometryCollection transformIntoPointGeometryCollection(final Geometry geom) {
   ...
   * @return a geometry collection
   */
  private static GeometryCollection transformIntoPointGeometryCollection(final GeometryCollection gc) {
    final UniqueCoordinateArrayFilter filter = new UniqueCoordinateArrayFilter();
    gc.apply(filter);
    final Coordinate[] coord = filter.getCoordinates();
...
private Geometry concaveHull() {
    ...
    final Collection<com.vividsolutions.jts.triangulate.quadedge.Vertex> qeVertices = qes.getVertices(false);
    ...
    final TreeMap<QuadEdge, Double> qeSorted = new TreeMap<QuadEdge, Double>(dc);
    ...
    for (final QuadEdge qe : qeSorted.keySet()) {
      ...
      final Integer idS = this.coordinates.get(s.p0);
      final Integer idD = this.coordinates.get(s.p1);
      final Vertex oV = this.vertices.get(idS);
      final Vertex eV = this.vertices.get(idD);
    ...
    for (final QuadEdgeTriangle qet : qeTriangles) {
      ...
      final Edge edgeA = this.edges.get(this.segments.get(sA));
      final Edge edgeB = this.edges.get(this.segments.get(sB));
      final Edge edgeC = this.edges.get(this.segments.get(sC));

      final Triangle triangle = new Triangle(i, qet.isBorder() ? true : false);
    ...
    for (final Edge edge : this.edges.values()) {
    ...
    while (index != -1) {
      ...
      final int si = this.lengths.size();

      if (si != 0) {
        final Entry<Integer, Edge> entry = this.lengths.firstEntry();
    ...
    for (final Edge e : this.lengths.values()) {
      final LineString l = e.getGeometry().toGeometry(this.geomFactory);
      edges.add(l);
    }

    for (final Edge e : this.shortLengths.values()) {
      final LineString l = e.getGeometry().toGeometry(this.geomFactory);
      edges.add(l);
    }
    ...
    final LineString merge = (LineString) lineMerger.getMergedLineStrings().iterator().next();

    if (merge.isRing()) {
      final LinearRing lr = new LinearRing(merge.getCoordinateSequence(), this.geomFactory);
      final Polygon concaveHull = new Polygon(lr, null, this.geomFactory);
      return concaveHull;
    }
